...
void yousefs_simulation(int experiment_n, json conditions)
{
	...
	std::string save_path = "sim_" + std::to_string(experiment_n);
	...
	if (save_csv){ // but don't create file unless save_csv is on
		std::string csv_path_output = path_output + save_path + ".csv";
		...
		to_string_with_precision(t_max,2) << "," <<
	...
	while(iter < max_iter) // looping over time
	{
		...
		if (iter % interval_write == 0){
			if (save_vtk){g_dist.write_frame(path_output + save_path, iter, FORMAT_BINARY);}
			if (save_hdf5){g_dist.save(path_output + save_path + std::to_string(iter) + ".hdf5");}
			if (save_mass){monitor_absolute_mass_over_region<CONC_N, PHI_N>(g_dist, emb_boundary, m_initial, p_volume, t, iter, path_output, save_path + "_mass.csv");}
